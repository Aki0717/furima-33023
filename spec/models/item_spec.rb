...
RSpec.describe Item, type: :model do
  ...
  describe "商品の出品" do
    context "商品の出品ができる" do
      ...
      it "category_idが2以上11以下だと登録できる" do
        @item.category_id = 4
        expect(@item).to be_valid
      end
      it "condition_idが2以上7以下だと登録できる" do
        @item.condition_id = 5
        expect(@item).to be_valid
      end
      it "delivery_fee_idが2以上3以下だと登録できる" do
        @item.delivery_fee_id = 5
        expect(@item).to be_valid
      end
      it "prefecture_idが2以上48以下だと登録できる" do
        @item.prefecture_id = 5
        expect(@item).to be_valid
      end
      it "days_to_ship_idが2以上4以下だと登録できる" do
        @item.days_to_ship_id = 5
        expect(@item).to be_valid
      end
      it "priceが300以上9999999未満だと登録できる" do
        @item.price = 500
        expect(@item).to be_valid
      end
    end
